...
if __name__ == '__main__':
    ...
    base_dir = osp.join('output', args.method+'_finetune', args.dataset, args.backbone)
        ...
        for source in domains:
            for target in domains:
                if source != target:
                    ...
                    src = source[0]
                    tgt = target[0]
                    load = f'./output/{args.method}/{args.dataset}/resnet50/{source}_to_{target}/{str(i+1)}/last.pth'

                    os.system(f'python finetune.py '
                              f'configs/{data_file}/src_{src}/finetune_{tgt}.py '
